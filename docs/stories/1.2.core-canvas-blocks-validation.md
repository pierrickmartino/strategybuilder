...
- `poetry run pytest`
- `pnpm --filter @strategybuilder/web lint`
- `pnpm --filter @strategybuilder/web test`

### Completion Notes List

- Delivered shared block catalog + validation contracts powering both FastAPI and Next.js flows.
- Implemented autosaving strategy version API with revert support and wired React Query/Zustand integration.
- Rebuilt canvas UI with drag/drop, inline badges, configuration inspector, and version history controls.
- Added targeted unit/integration/E2E coverage for store behavior, UI validation, and designer workflow.

### File List

- apps/api/app/api/routers/strategy_versions.py
- apps/api/app/services/graph_validation_service.py
- apps/api/app/services/strategy_version_service.py
- apps/api/tests/test_strategy_versions.py
- packages/shared/src/canvas.ts
- packages/shared/src/data/block-definitions.json
- apps/web/src/stores/useStrategyCanvas.ts
- apps/web/src/components/canvas/StrategyCanvas.tsx
- apps/web/src/components/canvas/CanvasInspector.tsx
- apps/web/src/components/canvas/CanvasNode.tsx
- apps/web/src/hooks/use-strategy-versions.ts
- apps/web/tests/integration/StrategyCanvas.test.tsx
- tests/e2e/canvas-designer.spec.ts
